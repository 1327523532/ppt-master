...
| Icon library | `${SKILL_DIR}/templates/icons/` | See `${SKILL_DIR}/templates/icons/README.md`; search icons on demand with `grep <keyword> templates/icons/index/<library>.txt` (full SVGs are fetched on demand, not shipped) |
...
**Default — Lenovo brand + Lenovo-Light deck.** Pre-apply two paths into Step 3: `templates/decks/Lenovo-Light/` (kind: deck) and `templates/brands/lenovo/` (kind: brand). Copy both into `<project_path>/templates/` per the dispatch below, lock identity / structure / middle segments per the existing fusion table, then proceed directly to Step 4. Do NOT query any `*_index.json`. Do NOT ask the user. Do NOT proactively suggest an alternative template, hint at one, or fuzzy-match content to a different template directory.

**Override flow triggers ONLY on explicit directory paths** supplied by the user in their initial message. The override rule is mechanical, not interpretive:

| User input contains | Step 3 action |
|---|---|
| No template-related input (default) | Apply `templates/decks/Lenovo-Light/` + `templates/brands/lenovo/` |
| One or more explicit template directory paths (overrides the default) | Read each spec's `kind`, dispatch per the kind matrix below, fuse if multiple. The Lenovo brand stays only when no `kind: brand` path is given in the override. |
| Style description ("麦肯锡风" / "极简风") or vague intent ("想做一份好看的") | Free design: skip Step 3 entirely; Strategist invents the visual system from the source |

There is no slug matching, no name lookup, no fuzzy resolution. A name without a path does not trigger an override — the user must give a path the AI can `cd` into.

> Style descriptions ("麦肯锡风格" / "Keynote 风" / "极简风" / etc.) never override into a template path. They flow into Strategist's Eight Confirmations as a style brief (color / typography / tone in confirmations e–g) and collapse the default into free design.

> Bare names "联想" / "Lenovo" do NOT change behavior — they ARE the default. A bare name that names a different template (e.g. "anthropic", "招商银行") still does NOT trigger an override; the user must give a path. AI must not "helpfully" resolve a name to a path.

> "What templates exist?" is out-of-band Q&A — answer by listing entries from `brands_index.json` / `layouts_index.json` / `decks_index.json` together with their paths. Listing alone does not advance the pipeline; the user must send a path back to override the default.
...
🚧 **GATE**: Step 3 complete; default Lenovo brand + Lenovo-Light deck applied, or (if user supplied an explicit template path) the override was dispatched (or fused) into `<project_path>/templates/` before advancing.
