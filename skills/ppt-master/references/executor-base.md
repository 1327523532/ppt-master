...
1a. **Brand-shell template** (template's `design_spec.md` frontmatter has `template_role: brand_shell`) → inherit brand background, logo, rules, and safe zones only. Do not add large opaque/translucent panels, frosted rectangles, or default card containers over cover/chapter backgrounds unless the user explicitly asks for that style.
...
- **PPTX-safe text fitting**: do not rely on PowerPoint to wrap or shrink text after export. Any title, subtitle, caption, bullet, axis label, or prose line that approaches the available width MUST be manually broken in SVG with `<tspan x="..." dy="...">` lines before export. As a quick self-check, estimate CJK glyphs at about `font-size` px each and Latin at about `0.55 * font-size`; if `x + estimated_width` reaches the safe right edge, split the text. Long CJK headings should normally be 2 lines, not one oversized `<text>` element.
...
**Anchor-page text rule**: cover, chapter, and ending pages use the largest font sizes, so they are the highest-risk pages for PPTX line drift. Hand-break cover titles and long takeaway sentences into visually intentional lines. Never place a long Chinese/English mixed heading as one single `<text>` merely because it fits in the browser preview.
