...
> Note: the `rendering/`, `palette/`, and `type/` sample PNGs are **not** distributed in the repo (removed to save space). The manifests are kept — regenerate the images with the `image_gen.py --manifest` command below before using them as a side-by-side gallery.
