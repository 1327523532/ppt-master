...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--daemon',
        action='store_true',
        help='Start the preview server in the background and return immediately',
    )
    parser.add_argument(
        '--wait-ready',
        action='store_true',
        help='With --daemon, wait until /api/slides responds before returning',
    )
    parser.add_argument(
        '--ready-timeout',
        type=float,
        default=15.0,
        help='Seconds to wait for --wait-ready (default: 15)',
    )
    parser.add_argument(
        '--live',
        action='store_true',
...
def _wait_until_ready(url: str, proc: subprocess.Popen | None, timeout: float) -> int:
    deadline = time.monotonic() + timeout
    probe_url = f'{url.rstrip("/")}/api/slides'
    last_error = ''
    while time.monotonic() < deadline:
        if proc is not None and proc.poll() is not None:
            logger.error('preview server exited before ready (code=%s)', proc.returncode)
            return 1
        try:
            with urllib.request.urlopen(probe_url, timeout=1.0) as resp:
                if resp.status == 200:
                    logger.info('preview server ready: %s', url)
                    return 0
                last_error = f'HTTP {resp.status}'
        except (OSError, urllib.error.URLError) as exc:
            last_error = str(exc)
        time.sleep(0.2)
    logger.error('preview server not ready after %.1fs: %s', timeout, last_error)
    return 1
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.daemon:
        ...
        if existing and _process_alive(int(existing.get('pid', 0))):
            existing_port = existing.get('port', args.port)
            url = f'http://localhost:{existing_port}'
            logger.info('preview server already running: %s (pid=%s)', url, existing.get('pid', '?'))
            if args.wait_ready:
                return _wait_until_ready(url, None, args.ready_timeout)
            return 0

        port = _find_free_port(args.port)
        log_path = project_path / '.preview' / 'server.log'
        ...
        cmd = [
            ...
            str(port),
            '--timeout',
            str(args.timeout if args.timeout is not None else (7200 if args.live else 900)),
        ]
        if args.no_browser:
        ...
        logger.info('started preview server in background: %s (pid=%s)', url, proc.pid)
        logger.info('log: %s', log_path)
        if args.wait_ready:
            return _wait_until_ready(url, proc, args.ready_timeout)
        return 0
