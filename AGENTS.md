...
>
> Visual self-check: **default-on** between Executor (SKILL.md Step 6) and post-processing (Step 7). The main pipeline auto-invokes [`visual-review`](skills/ppt-master/workflows/visual-review.md) unless one of the smart-skip conditions applies: (a) user explicit opt-out (`skip visual review` / `跳过视觉自检` / `不要视觉评审`), (b) `playwright` / chromium missing — log `playwright_missing` and continue, (c) deck has fewer than 3 pages — log `deck_too_small` and continue, (d) `spec_lock.md` carries `visual_review: off`. Failure handling: hard rubric hits are fixed in place by subagents (backups at `<project>/.review/backup/<page>.iter<N>.svg`); soft hits are aggregated into a per-page report and do not block Step 7. The trigger is no longer user request — the default is on, opt-out is the exception. Full gate semantics: [`workflows/visual-review.md`](skills/ppt-master/workflows/visual-review.md).
