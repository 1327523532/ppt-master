...
>
> Visual self-check: run by default between the executor and post-processing steps via the standalone [`visual-review`](skills/ppt-master/workflows/visual-review.md) workflow — every deck benefits from the per-page rubric pass. The user may opt out in chat (e.g. "跳过视觉自检", "skip visual review", "不要跑视觉回看") before Step 7 begins; once skipped, no further ask is needed. Do not infer or recommend running or skipping it from deck size, model identity, or any other signal — only an explicit opt-out changes the default.
